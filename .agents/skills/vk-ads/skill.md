...
Workflow-гайд для управления рекламой VK через единый MCP-сервер `lidfly` (v3).

**MCP-сервер:** `lidfly` — единый эндпоинт v3 (Директ, VK, Вордстат, Workspace). Инструменты VK — с префиксом `vk_`; задачи и напоминания — через Workspace.
