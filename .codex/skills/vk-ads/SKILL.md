...
**MCP-сервер:** `vk-ads` — отдельный от `yandex-direct`. 76 инструментов с префиксом `vk_`; задачи и напоминания ведутся через Workspace MCP.
...
Любой workflow этого скилла заверши итогом в чат: что создано, проверено или изменено, период и ключевые цифры если есть, вывод, применённые или предлагаемые действия, обновлённую запись в Workspace если она менялась. Workspace и технические логи не заменяют ответ пользователю.
...
4. **Действия:**
   ...
   - Записать результат в Workspace MCP: цифры за период — `workspace_save_analytics_snapshot`, изменение и вывод — `workspace_record_decision`.
...
| Яндекс Директ (если ведётся параллельно) | скилл `yandex-direct-campaign-builder` |
