...
Любой workflow этого скилла заверши итогом в чат: какие запросы проверены, какие позиции изменились, что это значит для CTR/CPA, что делать дальше и обновлённую запись в Workspace если она менялась. CSV-история и Workspace не заменяют ответ пользователю.
...
## Интеграция с MCP `lidfly`

Для расхода и статистики кампаний используй MCP `lidfly`:
...
- **yandex-direct-campaign-builder** — управление кампаниями, ставками, объявлениями
- **demand-research** — если конкурентов стало больше, исследовать новые ниши через анализ спроса

## Запись результата в Workspace

После проверки позиций зафиксируй результат в Workspace MCP:

1. Сохрани слепок позиций через `workspace_save_campaign_snapshot`, например:
```markdown
### Позиции в выдаче
...
2. Запиши вывод/действие через `workspace_record_decision`, если позиция значительно изменилась
