...
**MCP-сервер:** `lidfly` — Wordstat доступен через единый эндпоинт v3.

---

## Правило финального ответа

Любой workflow этого скилла заверши итогом в чат: что исследовано, какие выводы по спросу или интенту, что добавить, что минусовать, где риск каннибализации или сезонности, следующий шаг и обновлённую запись в Workspace если она менялась. CSV, таблицы и Workspace не заменяют ответ пользователю.
...
| `wordstat_user_info` | Доступность Search API, квота и остаток бесплатного месячного пакета |
...
5. **Записать результаты в Workspace MCP**: `workspace_save_document` (исследование/семантика) и `workspace_record_decision` (вывод).
...
- **Результаты всех исследований** — сохранять в Workspace MCP через `workspace_save_document` с датой и выводами, а ключевые выводы и рекомендации обязательно дублировать в чат.
